package com.example.ratingimages_makarevich;
...
public class MainActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        imageButtonLike.setEnabled(false); // Выключаем кнопку "Лайк" при запуске приложения

        imageButtonDislike = findViewById(R.id.imageButtonDislike);
        imageButtonDislike.setEnabled(false);   // Выключаем кнопку "Дизлайк" при запуске приложения
        ...
        imageButtonDownload = findViewById(R.id.imageButtonDownload);
        imageButtonDownload.setEnabled(false); // Выключаем кнопку "Скачать" при запуске приложения

        imageButtonLink = findViewById(R.id.imageButtonLink);
        imageButtonLink.setEnabled(false); // Выключаем кнопку "Перейти к источнику" при запуске приложения
    }

    public void GetSearchText(View view)
    {
        EditText editText = findViewById(R.id.editMessageForSearch);
        imageButtonLink.setEnabled(false);
        ...
        if (!userInput.isEmpty())
        {
            ...
            Picasso.get().load(R.drawable.rabbit).into(imageView);
        }
    }
    ...
    private Pair<String, String> loadImageFromUrl(List<String> imageUrls, List<String> pageUrls, int index)
    {
        ...
        imageButtonLink.setEnabled(true);
    ...
    {
        Toast.makeText(MainActivity.this, "Нравится (Макаревич)", Toast.LENGTH_SHORT).show();

        imageButtonLike.setEnabled(false); // Отключаем кнопку после нажатия
        new Handler().postDelayed(() -> {
            imageButtonLike.setEnabled(true); // Включаем кнопку после таймаута
        }, 3500); // 3.5с
    }

    public void clickDisLike(View view)
    {
        Toast.makeText(MainActivity.this, "Не нравится (Макаревич)", Toast.LENGTH_SHORT).show();

        imageButtonDislike.setEnabled(false); // Отключаем кнопку после нажатия
        new Handler().postDelayed(() -> {
            imageButtonDislike.setEnabled(true); // Включаем кнопку после таймаута
        }, 3500); // 3.5с
    }
    ...
    {
        imageButtonDownload.setEnabled(false); // Отключаем кнопку после нажатия
        new Handler().postDelayed(() -> {
            imageButtonDownload.setEnabled(true); // Включаем кнопку после таймаута
        }, 5000); //5с
    ...
    {
        imageButtonLink.setEnabled(false); // Отключаем кнопку после нажатия
        new Handler().postDelayed(() -> {
            imageButtonLink.setEnabled(true); // Включаем кнопку после таймаута
        }, 5000); //5с
